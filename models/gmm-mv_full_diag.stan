...
// GMM with QR decomposition, diagonal vcov, and mean-variance relation
...
transformed parameters {
  ...
  for (g in 1:G) {
    ...
    sigmas[, g] = exp(intercept_sigma[use_sigmas ? g : 1] + mus[, g] * cv[g]);
  }
}
...
model {
  ...
  if (use_priors) {
    ...
    to_vector(sdz0_mu) ~ student_t(prior_sdzMu_nu, 0, prior_sdzMu_sigma);
