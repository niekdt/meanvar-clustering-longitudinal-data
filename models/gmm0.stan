// random-intercept GMM with QR decomposition
...
model {
  ...
  for (g in 1:G) {
    r_mu[g] ~ normal(alpha[g], sdz_mu);    
  }
  
  for (i in 1:I) {
    for (g in 1:G) {
      ps[g] = normal_id_glm_lpdf(yc[offsets[i]:(offsets[i+1] - 1)] | Q[offsets[i]:(offsets[i+1] - 1),], r_mu[g][i], b_mu[,g], sigma);
    }
...
generated quantities {
  ...
  
  {
    ...
    for (i in 1:I) {
      ...
      pst = ps + log_theta;
      log_lik[i] = log_sum_exp(pst);
      pp[,i] = exp(pst - log_lik[i]);
    }
  }
}
